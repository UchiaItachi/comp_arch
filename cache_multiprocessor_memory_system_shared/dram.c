...
uns64   dram_access_sim_rowbuf(DRAM *dram,Addr lineaddr, Flag is_dram_write){
  ...
  const uns32 numLinesInRowBuf = ROWBUF_SIZE / CACHE_LINESIZE;
  const uns32 numOfLineBits = log2(numLinesInRowBuf);
  const uns32 numOfBankBits = log2(DRAM_BANKS);

  const uns32 lineNum = lineaddr & ((0x1 << numOfLineBits) - 1);
  const uns32 bankNum = (lineaddr >> numOfLineBits) & ((0x1 << numOfBankBits) - 1);

  const uns64 rowNum = (lineaddr >> (numOfBankBits + numOfLineBits));
