...
int32_t  BPRED_POLICY = 0; // 0:Perf 1:AlwaysTaken 2:Gshare
...
int main(int argc, char *argv[])
{
  ...
  //--------------------------------------------------------------------    
  for (way = 1; way < argc; way++)
  {
    if (argv[way][0] == '-')
    {
      if (!strcmp(argv[way], "-h") || !strcmp(argv[way], "-help"))
      {
        die_usage();
      }


      else if (!strcmp(argv[way], "-pipewidth"))
      {
        if (way < argc - 1)
        {
          PIPE_WIDTH = atoi(argv[way + 1]);
          way += 1;
        }
      }

      else if (!strcmp(argv[way], "-bpredpolicy"))
      {
        if (way < argc - 1)
        {
          BPRED_POLICY = atoi(argv[way + 1]);
          way += 1;
        }
      }

      else if (!strcmp(argv[way], "-enablememfwd"))
      {
        ENABLE_MEM_FWD = 1;
      }

      else if (!strcmp(argv[way], "-enableexefwd"))
      {
        ENABLE_EXE_FWD = 1;
      }
    }
  ...
  if ((tr_file = fopen(tr_filename, "r")) == NULL)
  {
