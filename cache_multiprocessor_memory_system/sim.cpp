...
uint64_t REPL_POLICY = 0; // 0:LRU 1:RAND
...
void die_usage()
{
  ...
  printf("   Options\n");
  printf(
    "      -mode            <num>    Set mode of the simulator[1:PartA, 2:PartB, 3:PartC 4:PartD 5:PartE]  (Default: 1)\n");
  ...
  for (ii = 1; ii < argc; ii++)
  {
    if (argv[ii][0] == '-')
    {
      ...
      else if (!strcmp(argv[ii], "-mode"))
      {
        if (ii < argc - 1)
        {
          SIM_MODE = (MODE_Enum) atoi(argv[ii + 1]);
          ii += 1;
        }
      }
