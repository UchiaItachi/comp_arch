...
void pipe_cycle_exe(Pipeline *p)
{
  ...
  EXEQ_cycle(p->pipe_EXEQ); // Cycle the exeq, to reduce wait time for each inst by 1 cycle

  int index = 0; // Transfer all finished entries from EXEQ to EX_latch
  while (1)
  {
...
void pipe_print_state(Pipeline *p)
{
  ...
  for (width_i = 0; width_i < PIPE_WIDTH; width_i++)
  {
    ...
    if (width_i == 0)
    {
      for (uint32_t i = 0; i < MAX_EXEQ_ENTRIES; ++i)
      {
        printf("  %d  ", (int)p->EX_latch[i].inst.inst_num);
      }
    }
    else
    {
